...
class TestTextChunker:
    ...
    def test_chunk_text_with_pages_basic(self):
        ...
        # chunk_text_with_pages принимает List[Tuple[int, str]]
        pages = [
            (1, "Это тестовый текст для проверки работы чанкера.")
        ]
    ...
    def test_chunk_text_with_pages_whitespace_only(self):
        ...
        pages = [(1, "   \n\n\t  ")]
        chunks = self.chunker.chunk_text_with_pages(pages, "test.pdf")
        # Пустой/пробельный текст не должен давать чанков
        assert isinstance(chunks, list)
        assert len(chunks) == 0
    ...
    def test_chunk_text_single_page(self):
        """Тест метода chunk_text для одной страницы."""
        chunks = self.chunker.chunk_text(
            "Тестовый текст для одного чанка.",
            "file.pdf",
            page_number=5
        )
        assert len(chunks) == 1
        assert chunks[0]["page_number"] == 5
        assert chunks[0]["file_name"] == "file.pdf"

    def test_chunk_text_empty_string(self):
        """Тест chunk_text с пустой строкой."""
        chunks = self.chunker.chunk_text("", "file.pdf", 1)
        assert chunks == []

    def test_chunk_text_none_like(self):
        """Тест chunk_text с пробельной строкой."""
        chunks = self.chunker.chunk_text("   \n  ", "file.pdf", 1)
        assert chunks == []